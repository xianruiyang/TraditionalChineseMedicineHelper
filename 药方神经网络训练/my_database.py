...
class herb_db:  #中药数据库
    ...
    def get_herbs(self,str=""):
        ...
        if str == "":
            return rt
        count = []
        inp = jieba.cut(str,cut_all = True)
        inp = '.'.join(inp)
        inp = inp.split('.')
        for i in range(len(rt)):
            count.append(self.herb_com(rt[i],inp))
        rt = sorted(rt,key = lambda x:-count[rt.index(x)])
        return rt
    ...
    def get_relations(self,str=""):
        ...
        inp = inp.split('.')
        for i in rt:
            count.append(self.relation_com(i,inp))
        rt = sorted(rt,key = lambda x:-count[rt.index(x)])
        return rt
    def get_recipes(self,str=""):
        ...
        inp = inp.split('.')
        for i in rt:
            count.append(self.recipe_com(i,inp))
        rt = sorted(rt,key = lambda x:-count[rt.index(x)])
        return rt
